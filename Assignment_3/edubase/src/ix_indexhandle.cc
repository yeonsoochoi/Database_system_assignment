...
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid, int detail) {
    ...
    if(node->GetKeysNum() == node->GetMaxKeys()){
        if(leftNode != NULL){
            IX_BTNode * nodeParent = path[level-1];
            
            FindLeaf(leftNode->LargestKey());
            IX_BTNode * nodeParentL = path[level-1];
            
            int posParentL = pathP[level-1];
            
            FindLeaf(pData);
            
            if(nodeParentL == nodeParent){
                if(leftNode->GetKeysNum() < leftNode->GetMaxKeys()){
                    
                    node->DistributeNode(leftNode);
                    nodeParent->DeleteNode(NULL, posParentL);
                    nodeParent->InsertNode(leftNode->LargestKey(), leftNode->GetNodeRID());
                }
            }
        }
    }
